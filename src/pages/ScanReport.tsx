...
import { ArrowLeft, Server, Calendar, ShieldAlert, Cpu, AlertTriangle, Info, CheckCircle2, Trash2, AlertOctagon, TrendingUp, Activity } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { apiClient, type Agent, type Scan, type ScanHistory, type ScanDiff } from '../api/client';
...
export function ScanReport() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [agentRes, scanRes, historyRes, diffRes] = await Promise.all([
        ...
        setHistory(historyRes as unknown as ScanHistory[]);
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-6 h-full pb-8">
      ...
      {/* Main Tabs (Tabs for Feature 13) */}
      <div className="flex border-b border-border text-sm font-semibold tracking-widest mt-2 mb-2">
        <button 
          onClick={() => setActiveMainTab('current')} 
          className={`px-6 py-3 border-b-2 flex items-center gap-2 transition-colors ${activeMainTab === 'current' ? 'border-primary text-primary' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
        >
          Current Findings
        </button>
        <button 
          onClick={() => setActiveMainTab('history')} 
          className={`px-6 py-3 border-b-2 flex items-center gap-2 transition-colors ${activeMainTab === 'history' ? 'border-primary text-primary' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
        >
