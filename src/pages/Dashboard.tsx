...
export function Dashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [statsRes, agentsRes] = await Promise.all([apiClient.get('/dashboard'), apiClient.get('/agents')]);
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-8">
      <header className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-widest text-primary uppercase">Command Center</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-8 flex flex-col gap-4">
            <h2 className="text-xl font-semibold tracking-wide text-zinc-100 uppercase">Agent Posture Overview</h2>
              ...
              {agents.length === 0 ? (
                ...
                <div className="flex flex-col gap-6">
                  {agents.map((agent) => {
                    ...
                    const colorClass =
                      agent.latest_hardening_index !== undefined ? getHardeningScoreBgColor(score) : 'bg-zinc-700';
                        ...
                        <div className="flex justify-between items-end">
                          <div className="flex items-center gap-2">
                            ...
                            <Link
                              ...
                              className="text-lg font-medium text-zinc-400 hover:text-primary/80 flex items-center gap-2"
                            >
...
function StatCard({
  ...
  return (
    <div className="flex flex-col bg-surface border border-border p-6 rounded-xl relative overflow-hidden group">
      ...
      <div className="mt-4 text-4xl font-black tracking-tight text-zinc-100">{value}</div>
